...
class AddEdit extends Component {
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.userDetail !== this.props.userDetail && this.props.userDetail && !this.state.changedProp) {
            const userDetail = {
                ...this.props.userDetail,
                changedProp: true,
                dueDate: Moment(this.props.userDetail.dueDate)._d,
            };
            this.setState(userDetail);
        }
    }
    ...
    render() {
        if (this.props.redirectTo) {
            swal("Success", "Your Details have been saved!", { icon: "success" });
            return <Navigate to={this.props.redirectTo} replace />;
        }

        if (this.props.userDetailError) {
            swal("Error", this.props.userDetailError, { icon: "error" });
            return <Navigate to={this.props.redirectTo} replace />;
        }

        const AddEditButton = !this.props.addEditLoading ? "Save" : "Please Wait ...";

        return (
            <CCard>
                <CCardHeader className="d-flex">
                    <div className="col-sm">User {this.state.name ? "Edit" : "Add"}</div>
                </CCardHeader>
                <CCardBody>
                    <CForm
                        className="row g-3"
                    ...
                    >
                        ...
                        <CCol xs={12}>
                            <CButton className="save-btn" type="submit">{AddEditButton}</CButton>
                        </CCol>
                    </CForm>
                </CCardBody>
            </CCard>
